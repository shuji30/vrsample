 ...
 *
 *   0.00  構え     胸の前で両手に持つ
 *   0.18  割る     両手を離し、右手を下から後ろへ回し始める
 *   0.40  振りかぶり 体を閉じ（左肩を相手へ）、左膝を軽く上げ、重心を後ろへ
 *   0.55  トップ   右手は頭の後ろ、左手は相手を指す
...
// トップ（頭の後ろ）からリリース（前）へは、頭の右横・上を回す。まっすぐ
// 結ぶと手が頭の中を通り、額に手を当てたように見えた
const RIGHT_HAND = [
  [0.00, [-0.07, 0.62, 0.26]],
  [0.18, [-0.12, 0.58, 0.18]],
  [0.40, [-0.30, 0.56, -0.16]],
...
const HIPS_YAW = [[0, 0], [0.30, -0.10], [0.52, -0.35], [0.62, -0.20], [0.72, 0.30], [1, 0.30]];
const CHEST_YAW = [[0, 0], [0.30, -0.15], [0.55, -0.50], [0.64, -0.30], [0.74, 0.55], [1, 0.45]];
...
export function createThrow(body, target, { onRelease }) {
  ...
  // 投げる腕の肘は外・少し下・少し後ろへ逃がす（スリークォーター気味）
  const rightPole = new THREE.Vector3().addScaledVector(left, -1).add(new THREE.Vector3(0, -0.35, 0)).addScaledVector(forward, -0.2).normalize();
